...
if st.sidebar.button("📘 打开模型介绍 Model Intro", use_container_width=True):
    with st.expander("模型介绍", expanded=True):
        st.markdown("""
        # 📚 模型介绍与参考文献
        ### ARIMA · BiLSTM · Transformer
        本项目使用 **BiLSTM** 和 **Transformer** 两种深度学习模型进行股票价格预测，
        并以传统统计模型 **ARIMA** 作为基准对比。

        ---
        ## 📈 1. ARIMA (Autoregressive Integrated Moving Average)
        **ARIMA** 是由 Box & Jenkins（1970）提出的经典时间序列统计模型：
        - **AR（自回归）**：用过去若干期的值预测当前值
        - **I（积分）**：对非平稳序列进行差分，使其平稳
        - **MA（移动平均）**：用过去误差修正当前预测
        ...
        **BiLSTM** 是对单向 LSTM 的扩展，同时从正向和反向处理序列：
        ...
        **Transformer** 由 Vaswani et al.（2017）提出，使用自注意力机制：
        - **多头注意力**：从多个角度捕捉依赖
        - **位置编码**：注入序列位置信息
        - **并行计算**，训练速度快
...
# ====================== 【你要的第二个按钮：项目报告（残差下面、保存上面）】 ======================
                st.divider()
                st.subheader("📄 项目报告 Project Report")
                if st.button("📄 打开完整项目报告", use_container_width=True):
                    with st.expander("项目报告", expanded=True):
                        ...
                        This project aims to predict future stock prices using historical financial data.
                        We compare three models: ARIMA, BiLSTM, Transformer.

                        ### 2. Models Used
                        1. **ARIMA**: Classic time-series model based on autoregression.
                        2. **BiLSTM**: Bidirectional recurrent neural network.
                        3. **Transformer**: Self-attention mechanism for long-range dependencies.
                        ...
                        - Transformer: d_model, attention heads
                        - ARIMA: p, d, q orders
                        ...
                        We also provide:
                        - Prediction vs actual curves
                        - Residual plots & distribution
                        - Training & validation loss curves

                        ### 5. Model Comparison & Interpretation
                        - **ARIMA**: Fast, stable, weak for nonlinear patterns.
                        - **BiLSTM**: Good at short-term trends, slow training.
                        - **Transformer**: Strong long-range modeling, needs more data.

                        ### 6. Personal Analysis
                        1. **Short-term Prediction: ARIMA performs strongest**
                           Highest accuracy on short-term, high-noise data.

                        2. **Medium-term Prediction: BiLSTM balanced**
                           Good at nonlinear dependencies but slight lag.

                        3. **Long-term Prediction: Transformer weaker on small data**
                           Over-smoothing, lag, noise sensitivity in real stock data.
